package no.novari.fint.consumer.models.undervisningsgruppe;
...
@Slf4j
@Api(tags = {"Undervisningsgruppe"})
@CrossOrigin
@RestController
@RequestMapping(name = "Undervisningsgruppe", value = RestEndpoints.UNDERVISNINGSGRUPPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class UndervisningsgruppeController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Undervisningsgruppe cache is disabled.");
        }
    ...
    @GetMapping
    public UndervisningsgruppeResources getUndervisningsgruppe(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getUndervisningsgruppeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Undervisningsgruppe cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<UndervisningsgruppeResource> resources;
    ...
    @PostMapping("/$query")
    public UndervisningsgruppeResources getUndervisningsgruppeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getUndervisningsgruppe(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private UndervisningsgruppeResources getUndervisningsgruppeByOdataFilter(
        ...
        Event event = new Event(
                ...
                TimeplanActions.GET_UNDERVISNINGSGRUPPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new UndervisningsgruppeResources();
    
        ArrayList<UndervisningsgruppeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<UndervisningsgruppeResource>>() {});
    ...
    public UndervisningsgruppeResource getUndervisningsgruppeBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<UndervisningsgruppeResource> undervisningsgruppe = cacheService.getUndervisningsgruppeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return undervisningsgruppe.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            UndervisningsgruppeResource undervisningsgruppe = objectMapper.convertValue(response.getData().get(0), UndervisningsgruppeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(undervisningsgruppe);
        }    
    }
