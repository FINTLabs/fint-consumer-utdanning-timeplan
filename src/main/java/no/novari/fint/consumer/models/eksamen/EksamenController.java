package no.novari.fint.consumer.models.eksamen;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Eksamen", value = RestEndpoints.EKSAMEN, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class EksamenController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private EksamenCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Eksamen cache is disabled.");
        }
    ...
    @GetMapping
    public EksamenResources getEksamen(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getEksamenByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Eksamen cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, TimeplanActions.GET_ALL_EKSAMEN, client);
        ...
        Stream<EksamenResource> resources;
    ...
    @PostMapping("/$query")
    public EksamenResources getEksamenByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getEksamen(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private EksamenResources getEksamenByOdataFilter(
        ...
        Event event = new Event(
                ...
                TimeplanActions.GET_EKSAMEN, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new EksamenResources();
    
        ArrayList<EksamenResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<EksamenResource>>() {});
    ...
    public EksamenResource getEksamenBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, TimeplanActions.GET_EKSAMEN, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<EksamenResource> eksamen = cacheService.getEksamenBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return eksamen.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            EksamenResource eksamen = objectMapper.convertValue(response.getData().get(0), EksamenResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(eksamen);
        }    
    }
