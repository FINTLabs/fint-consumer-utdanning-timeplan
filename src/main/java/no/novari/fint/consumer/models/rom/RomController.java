package no.novari.fint.consumer.models.rom;
...
public class RomController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Rom cache is disabled.");
        }
    ...
    public RomResources getRom(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getRomByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Rom cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public RomResources getRomByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getRom(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private RomResources getRomByOdataFilter(
        ...
        Event event = new Event(
                ...
                TimeplanActions.GET_ROM, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new RomResources();
    
        ArrayList<RomResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<RomResource>>() {});
    ...
    public RomResource getRomBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<RomResource> rom = cacheService.getRomBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return rom.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            RomResource rom = objectMapper.convertValue(response.getData().get(0), RomResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(rom);
        }    
    }
