package no.novari.fint.consumer.models.time;
...
import no.novari.fint.model.resource.utdanning.timeplan.TimeResources;
...
public class TimeController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Time cache is disabled.");
        }
    ...
    public TimeResources getTime(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getTimeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Time cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public TimeResources getTimeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getTime(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private TimeResources getTimeByOdataFilter(
        ...
        Event event = new Event(
                ...
                TimeplanActions.GET_TIME, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new TimeResources();
    
        ArrayList<TimeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<TimeResource>>() {});
    ...
    public TimeResource getTimeBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<TimeResource> time = cacheService.getTimeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return time.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            TimeResource time = objectMapper.convertValue(response.getData().get(0), TimeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(time);
        }    
    }
