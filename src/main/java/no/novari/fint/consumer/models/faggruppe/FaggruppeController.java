package no.novari.fint.consumer.models.faggruppe;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Faggruppe", value = RestEndpoints.FAGGRUPPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class FaggruppeController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private FaggruppeCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Faggruppe cache is disabled.");
        }
    ...
    @GetMapping
    public FaggruppeResources getFaggruppe(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFaggruppeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Faggruppe cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, TimeplanActions.GET_ALL_FAGGRUPPE, client);
        ...
        Stream<FaggruppeResource> resources;
    ...
    @PostMapping("/$query")
    public FaggruppeResources getFaggruppeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFaggruppe(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FaggruppeResources getFaggruppeByOdataFilter(
        ...
        Event event = new Event(
                ...
                TimeplanActions.GET_FAGGRUPPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FaggruppeResources();
    
        ArrayList<FaggruppeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FaggruppeResource>>() {});
    ...
    public FaggruppeResource getFaggruppeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, TimeplanActions.GET_FAGGRUPPE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<FaggruppeResource> faggruppe = cacheService.getFaggruppeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return faggruppe.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FaggruppeResource faggruppe = objectMapper.convertValue(response.getData().get(0), FaggruppeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(faggruppe);
        }    
    }
