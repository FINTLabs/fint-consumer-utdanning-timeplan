...
@Slf4j
@Service
@ConditionalOnProperty(name = "fint.consumer.cache.disabled.time", havingValue = "false", matchIfMissing = true)
public class TimeCacheService extends CacheService<TimeResource> {
    ...
    @Scheduled(initialDelayString = Constants.CACHE_INITIALDELAY_TIME, fixedRateString = Constants.CACHE_FIXEDRATE_TIME)
    ...
    public void onAction(Event event) {
        ...
        if (TimeplanActions.valueOf(event.getAction()) == TimeplanActions.UPDATE_TIME) {
            if (event.getResponseStatus() == ResponseStatus.ACCEPTED || event.getResponseStatus() == ResponseStatus.CONFLICT) {
                List<CacheObject<TimeResource>> cacheObjects = data
                    .stream()
