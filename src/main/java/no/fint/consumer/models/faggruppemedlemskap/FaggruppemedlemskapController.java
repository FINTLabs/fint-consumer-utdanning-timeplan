package no.fint.consumer.models.faggruppemedlemskap;
...
import no.fint.model.resource.utdanning.timeplan.FaggruppemedlemskapResources;
import no.fint.model.utdanning.timeplan.TimeplanActions;

@Slf4j
@Api(tags = {"Faggruppemedlemskap"})
@CrossOrigin
@RestController
...
public class FaggruppemedlemskapController {
    ...
    @GetMapping
    public FaggruppemedlemskapResources getFaggruppemedlemskap(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, TimeplanActions.GET_ALL_FAGGRUPPEMEDLEMSKAP, client);
        ...
        Stream<FaggruppemedlemskapResource> resources;
    ...
    public FaggruppemedlemskapResource getFaggruppemedlemskapBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, TimeplanActions.GET_FAGGRUPPEMEDLEMSKAP, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<FaggruppemedlemskapResource> faggruppemedlemskap = cacheService.getFaggruppemedlemskapBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return faggruppemedlemskap.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FaggruppemedlemskapResource faggruppemedlemskap = objectMapper.convertValue(response.getData().get(0), FaggruppemedlemskapResource.class);
