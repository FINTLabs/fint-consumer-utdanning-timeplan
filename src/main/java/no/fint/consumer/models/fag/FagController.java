...
@CrossOrigin
@RestController
@RequestMapping(name = "Fag", value = RestEndpoints.FAG, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class FagController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Fag cache is disabled.");
        }
    ...
    public FagResources getFag(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFagByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fag cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public FagResources getFagByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFag(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FagResources getFagByOdataFilter(
        ...
        Event event = new Event(
                ...
                TimeplanActions.GET_FAG, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FagResources();
    
        ArrayList<FagResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FagResource>>() {});
    ...
    public FagResource getFagBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<FagResource> fag = cacheService.getFagBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return fag.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FagResource fag = objectMapper.convertValue(response.getData().get(0), FagResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(fag);
        }    
    }
