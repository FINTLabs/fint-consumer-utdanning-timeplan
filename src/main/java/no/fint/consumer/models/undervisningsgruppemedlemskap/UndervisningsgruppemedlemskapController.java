...
public class UndervisningsgruppemedlemskapController {
    ...
    public UndervisningsgruppemedlemskapResources getUndervisningsgruppemedlemskap(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getUndervisningsgruppemedlemskapByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Undervisningsgruppemedlemskap cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public UndervisningsgruppemedlemskapResources getUndervisningsgruppemedlemskapByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getUndervisningsgruppemedlemskap(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private UndervisningsgruppemedlemskapResources getUndervisningsgruppemedlemskapByOdataFilter(
        ...
        Event event = new Event(
                ...
                TimeplanActions.GET_UNDERVISNINGSGRUPPEMEDLEMSKAP, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new UndervisningsgruppemedlemskapResources();
    
        ArrayList<UndervisningsgruppemedlemskapResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<UndervisningsgruppemedlemskapResource>>() {});
